...
from services import oauth, utils
...
class SocialRegisterController(BaseController):
    ...
    def twitter(self, request, response, network):
        ...
        tries = 5
        while True:
            result = oauth.fetchResponse(tokenRequest, network.base_url)
            tries -= 1
...
class SocialCallbackController(BaseController):
    ...
    def twitter(self, request, response, network):
        ...
        accessToken = oauth.fetchResponse(oauthRequest, network.base_url)
        ...
        if network.name == 'linkedin':
            profile_request_url = 'https://%s/v1/people/~:%s' % (network.base_url, settings.LINKED_IN_PROFILE_FIELDS)
            oauthRequest = oauth.makeOauthRequestObject(profile_request_url,
                                network.getCredentials(), token=oauth.OAuthToken.from_string(accessToken),
                                method='GET')
            ret = oauth.fetchResponse(oauthRequest, network.base_url, headers={'x-li-format': 'json'})
